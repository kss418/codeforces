...
void run(){
    ...
    for(int i = 1;i <= n;i++){
        ll now = a[i], use = 0; mul[now]++;
        if(es.prime(now)) {
            cnt[1]++; cnt[now]++;
            result += cnt[1] - mul[now];
        }
        ...
        for(auto& j : es.ret()){
            ...
            if(now % j) continue;
            result += cnt[now / j];
            if(es.prime(now / j) && j * j != now) result += cnt[j];
        }
